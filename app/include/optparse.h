...
#define ARGV_MAXARG_CNT 8
#define ARGV_MAXARG_LEN 16
static char _argv[ARGV_MAXARG_CNT][ARGV_MAXARG_LEN];

static void cmd2argv(char *cmd, void **argv, int *argc)
{
    int cnt = 0;
    char *psite = cmd;

    for (char *csite = strstr(cmd, " ");
            csite != NULL; csite = strstr(csite, " ")) {
        strncpy(_argv[cnt++], psite, csite - psite);
        while (csite[0] == ' ') {
            ++csite;
        }
        psite = csite;
    }
    strncpy(_argv[cnt++], psite, ARGV_MAXARG_LEN);
...
static void free_argv(void *argv)
{
    memset(argv, 0, ARGV_MAXARG_CNT * ARGV_MAXARG_LEN);
}
...
optparse(struct optparse *options, const char *optstring)
{
    ...
    next = (char *)(options->argv) + (options->optind + 1) * ARGV_MAXARG_LEN;
    switch (type) {
    case -1: {
        char str[2] = {0, 0};
        str[0] = option[0];
